...
- **`project-lint-core/src/scanners/dependabot.rs`**: Validates `.github/dependabot.yml` — ecosystem coverage, schedule intervals, group config, assignees/reviewers, and `github-actions` ecosystem presence when workflows exist. Warns when `.github/dependabot.yml` is missing and supports `--fix` to create ecosystem-aware defaults. Configurable via `[scanner_config.dependabot]`. Gated by the `dependabot` check name.
- **`project-lint-core/src/scanners/yamllint_config.rs`**: Checks for a `.yamllint` config file in the project root — warns when missing and supports `--fix` to create sensible defaults (`extends: default`, disabled `document-start`/`line-length`, `truthy.check-keys: false`). Optionally validates `extends:` field presence (info-level). Configurable via `[scanner_config.yamllint_config]` with `require_extends`. Gated by the `yamllint_config` check name.
...
- **`project-lint-core/src/scanners/shell_script.rs`**: Validates `*.sh` / `*.bash` files against the shell-scripting best-practices bundle — shebang form (`#!/usr/bin/env bash`), `set -euo pipefail` strict mode, guarded PATH additions, `exec` for final long-lived commands, dirty-git gates and dry-run support for destructive scripts, bounded timeouts, no hardcoded home paths, forbidden package-manager commands (`npx`/`bunx`/`yarn dlx`), and `devbox run --` usage in devbox projects. Configurable via `[scanner_config.shell_script]` with `require_shebang`, `require_strict_mode`, `forbid_hardcoded_home`, `forbidden_commands`, and `require_devbox_run`. Gated by the `shell_script` check name.
- **`project-lint-core/src/scanners/agents_md.rs`**: Validates the binding-contract structure of `AGENTS.md`, `CLAUDE.md`, and `AGENT.md` files — optional "Usage Protocol" and "JIT Index" sections (info), no absolute filesystem paths (warning), no AI tool attribution (error), and child-chain reference integrity (warning). Configurable via `[scanner_config.agents_md]` with `require_usage_protocol`, `require_jit_index`, `check_child_chain`, and `attribution_patterns`. Gated by the `agents_md` check name.
- **`project-lint-core/src/scanners/path_hygiene.rs`**: Cross-cutting scanner that validates all committed text files for path hygiene — no absolute home paths `/Users/...` or `/home/...` (warning, exempt files like `.envrc`/`.gitignore`/`*.env`), no AI tool attribution boilerplate (error), and no AI signature comments in source headers (error). Configurable via `[scanner_config.path_hygiene]` with `check_absolute_home`, `check_ai_attribution`, `check_ai_signature`, `exempt_files`, `attribution_patterns`, and `signature_patterns`. Uses the centralized exclusion list. Gated by the `path_hygiene` check name.
- **`project-lint-core/src/scanners/python_config.rs`**: Validates `pyproject.toml` for modern Python packaging conventions — `[build-system]` presence, `uv`/`ruff` tooling, no `==` pinning, `requires-python` declaration, ruff configuration, and absence of legacy `setup.py` / `requirements.txt`. Configurable via `[scanner_config.python_config]` with `required_tools`, `forbid_setup_py`, `forbid_requirements_txt`. Gated by the `python_config` check name.
- **`project-lint-core/src/scanners/go_config.rs`**: Validates `go.mod` / `go.sum` for Go module conventions — Go version specification (1.22+), no local filesystem `replace` directives, no `// indirect` deps in `go.mod`, and `go.sum` presence. Configurable via `[scanner_config.go_config]` with `require_go_sum`, `forbid_local_replace`. Gated by the `go_config` check name.
- **`project-lint-core/src/scanners/gradle_config.rs`**: Validates `build.gradle` / `settings.gradle` for Gradle/JVM conventions — no dynamic `+` versions, no `SNAPSHOT` versions, `repositories` block presence, `pluginManagement` in settings, wrapper presence, and `gradlew` execute permission. Configurable via `[scanner_config.gradle_config]` with `forbid_dynamic_versions`, `forbid_snapshots`, `require_wrapper`. Gated by the `gradle_config` check name.
- **`project-lint-core/src/scanners/rust_conventions.rs`**: Enforces Rust development practices — no `dbg!`/debug `println!` outside tests, no `.unwrap()` in library code, no `unsafe` blocks in non-`build.rs` source, no forbidden crates in `Cargo.toml`, and Cargo.toml content validation (edition 2021, `description`/`license`/`repository` fields, no floating `*` deps, workspace deps, criterion in dev-deps). Configurable via `[scanner_config.rust_security]` with `forbidden_crates`, `require_edition_2021`, `require_license`, `forbid_floating_deps`. Gated by the `rust_conventions` check name.
- **`project-lint-core/src/scanners/terraform_lint.rs`**: Validates Terraform `.tf` / `.tfvars` / `.tf.json` files — hardcoded secrets, variable/output descriptions and types, provider version pinning, backend configuration, default_tags usage, and `.terraform.lock.hcl` presence. Forward-looking (zero instances today, planned). Configurable via `[scanner_config.terraform_lint]`. Gated by the `terraform_lint` check name. Silent when no `.tf` files exist.
- **`project-lint-core/src/scanners/pulumi_lint.rs`**: Validates `Pulumi.yaml` / `Pulumi.*.yaml` files — required `name:` and `runtime:` fields, `config:` section presence, plaintext secrets in config, and `description:` field. Forward-looking (zero instances today, planned). Configurable via `[scanner_config.pulumi_lint]`. Gated by the `pulumi_lint` check name. Silent when no `Pulumi.*.yaml` files exist.
- **`project-lint-core/src/scanners/ansible_lint.rs`**: Validates Ansible playbooks (in `ansible/`, `playbooks/`, `roles/` dirs) and `ansible.cfg` — `become: true` at play level, hardcoded vault passwords, missing task names, `host_key_checking = False`, and `vault_password_file` with committed paths. Configurable via `[scanner_config.ansible_lint]`. Gated by the `ansible_lint` check name. Silent when no Ansible files exist.
- **`project-lint-core/src/scanners/jinja_template.rs`**: Validates Jinja2 templates (`.j2` / `.jinja2`) — secret literal variable names, forbidden filters (`eval`, `exec`), absolute paths in `{% include %}` / `{% extends %}`, and raw includes. Configurable via `[scanner_config.jinja_template]`. Gated by the `jinja_template` check name. Silent when no `.j2` / `.jinja2` files exist.
- **`project-lint-core/src/scanners/sql_migration.rs`**: Validates `*.sql` migration files in `migrations/`, `db/migrations/`, and `sql/migrations/` directories — checks sequential numbering with no gaps, filename descriptions, `DROP TABLE` without `IF EXISTS`, `DROP DATABASE`, `SELECT *` in views, unclosed `BEGIN`/`COMMIT` transactions, idempotency guards, and hardcoded passwords/tokens in `INSERT` statements. Gated by the `sql_migration` check name. Configurable via `[scanner_config.sql_migration]`.
- **`project-lint-core/src/scanners/protobuf_lint.rs`**: Validates `*.proto` files — checks `syntax = "proto3"`, `package` declaration, snake_case field names, PascalCase message names, enum zero-value naming (`_UNSPECIFIED`/`_UNKNOWN`), reserved field number collisions, deprecated fields without reserved numbers, and unused imports. Gated by the `protobuf_lint` check name. Configurable via `[scanner_config.protobuf_lint]`.
- **`project-lint-core/src/scanners/prisma_schema.rs`**: Validates `*.prisma` schema files — checks `datasource` block with `provider` and `env("DATABASE_URL")` url, `generator` block with `provider`, model `@id` fields, `createdAt`/`updatedAt` timestamps (optional), `@@index` for `@relation` foreign keys, `onDelete: Cascade` warnings, and hardcoded connection strings/passwords. Gated by the `prisma_schema` check name. Configurable via `[scanner_config.prisma_schema]`.
- **`project-lint-core/src/scanners/binary_validation.rs`**: Validates binary files (`*.png`, `*.jpg`, `*.gif`, `*.svg`, `*.pdf`, `*.mp4`, archives, compiled artifacts, etc.) — flags binaries that should use Git LFS (default 1 MB threshold), oversized binaries (default 10 MB error limit), binaries committed into source directories, duplicate binaries (identical SHA-256 content), committed archives, and committed compiled artifacts. Configurable via `[scanner_config.binary_validation]` with `lfs_threshold_bytes`, `max_size_bytes`, per-rule toggles (`check_lfs`, `check_oversized`, `check_source_dir`, `check_duplicate`, `check_archive`, `check_compiled`), `extra_binary_extensions`, and `extra_source_dirs`. Gated by the `binary_validation` check name.
- **`project-lint-core/src/utils.rs` (centralized exclusion list)**: Shared utility that all WalkDir-based scanners use to skip build artifacts, dependency directories, and VCS internals (`node_modules/`, `target/`, `dist/`, `build/`, `.next/`, `.turbo/`, `.nuxt/`, `.svelte-kit/`, `.git/`, `.devbox/gen/`, `.cache/`, `coverage/`, and configurable `vendor/`). Configurable via `[scanner_config.exclusion]` with `extra_excludes`, `allow_vendor`, and `max_depth`. Provides `DEFAULT_EXCLUDED_DIRS`, `build_exclusions()`, `is_excluded_rel()`, `is_excluded_entry()`, and `walk_project()`. All WalkDir-based scanners (`rust_conventions`, `dockerfile_lint`, `compose_lint`, `skill_markdown`, `magic_numbers`, `vault_security`, `file_naming`, `nix_flake`, `devbox_json`, `nix_shell`, `envrc_content`, `github_workflow`, `justfile_content`, `makefile_content`, `process_compose`, `nx_project`, `shell_script`, `agents_md`, `path_hygiene`, `python_config`, `go_config`, `gradle_config`, `terraform_lint`, `pulumi_lint`, `ansible_lint`, `jinja_template`, `sql_migration`, `protobuf_lint`, `prisma_schema`, `binary_validation`) use this utility via `with_exclusions()` / `with_config_and_exclusions()` constructors.
