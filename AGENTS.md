...
- **`project-lint-core/src/scanners/skill_markdown.rs`**: Validates `SKILL.md` files in the skills-src wrapper pattern — body line limit (default 80, configurable via `[scanner_config.skill_markdown]`), `scripts/refresh.sh` presence, required frontmatter fields (`name`, `description`, `version`), and unquoted-colon detection in the `description:` field (rule: `skill-frontmatter-unquoted-colon`, severity: error). Gated by the `skill_markdown` check name.
...
- **`project-lint-core/src/scanners/markdown_frontmatter.rs`**: Validates YAML frontmatter in all `.md` files — required fields (title, synopsis, tags), frontmatter delimiters, ADR-specific rules (adr-id format, status, date, version) for files under `internal-docs/adr` or `docs-internal/adr`, and unquoted-colon detection in any scalar frontmatter value (rule: `md-frontmatter-unquoted-colon`, severity: error). Configurable via `[scanner_config.markdown_frontmatter]` (require_frontmatter, adr_dirs). Gated by the `markdown_frontmatter` check name.
