...
## AGENTS.md Validation + Path Hygiene (2026-09-01)

**PRD**: [docs-internal/requirements/20260901-agents-md-validation.md](requirements/20260901-agents-md-validation.md)

### Shipped

- **`agents_md` scanner** (`project-lint-core/src/scanners/agents_md.rs`):
  Validates the binding-contract structure of `AGENTS.md`, `CLAUDE.md`, and
  `AGENT.md` files. Five rules:
  - `agents-md-usage-protocol` (info) — optional "Usage Protocol" section
  - `agents-md-jit-index` (info) — optional "JIT Index" / table of contents
  - `agents-md-no-absolute-paths` (warning) — no `/Users/...` or `/home/...`
  - `agents-md-no-ai-attribution` (error) — no AI tool attribution boilerplate
  - `agents-md-child-chain` (warning) — referenced child `AGENTS.md` files must exist
  Configurable via `[scanner_config.agents_md]`. Gated by `agents_md` check name.
  Uses centralized exclusion list. Distinct from `markdown_frontmatter` (which
  validates generic `.md` YAML frontmatter, not agent-contract body structure).

- **`path_hygiene` scanner** (`project-lint-core/src/scanners/path_hygiene.rs`):
  Cross-cutting scanner validating all committed text files. Three rules:
  - `no-absolute-home-paths` (warning) — no `/Users/<user>/` or `/home/<user>/`
    (exempt: `.envrc`, `.gitignore`, `*.env`)
  - `no-ai-attribution` (error) — no "Generated with [Devin]", "Co-Authored-By: Devin"
  - `no-ai-signature-comments` (error) — no "AI-generated", "Generated by Claude", etc.
  Configurable via `[scanner_config.path_hygiene]`. Gated by `path_hygiene` check name.
  Uses centralized exclusion list. Scans text files only (by extension + special names),
  skips files over 512 KB.

### Tests

- `agents_md`: 18 unit tests (valid file, missing usage protocol, missing JIT index,
  absolute paths macOS/Linux, AI attribution, child chain missing/present/disabled,
  CLAUDE.md scanning, empty file, no AGENTS.md, helper functions)
